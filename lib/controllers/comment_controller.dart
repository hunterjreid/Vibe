...
class CommentController extends GetxController {
  ...
  getComment() async {
    _comments.bindStream(
      firestore.collection('videos').doc(_postId).collection('comments').snapshots().map(
        (QuerySnapshot query) {
          // Mapping the QuerySnapshot to a List of Comment objects
          List<Comment> retValue = [];
          for (var element in query.docs) {
            retValue.add(Comment.fromSnap(
                element)); // Creating a Comment object from each document snapshot and adding it to the list
          }
          return retValue;
        },
      ),
    ...
    try {
      if (commentText.isNotEmpty) {
        DocumentSnapshot userDoc =
            await firestore.collection('users').doc(authController.user.uid).get(); // Getting the user document
        var allDocs = await firestore
            .collection('videos')
            .doc(_postId)
            .collection('comments')
            .get(); // Getting all comment documents for the post
        ...
        await firestore.collection('videos').doc(_postId).collection('comments').doc('Comment $len').update({
          'commentBy': FieldValue.arrayUnion(
              [authController.user.uid]), // Adding the user's ID to the 'commentBy' field in the comment document
        });

        DocumentSnapshot doc = await firestore.collection('videos').doc(_postId).get(); // Getting the post document
        await firestore.collection('videos').doc(_postId).update({
          'commentCount':
              (doc.data()! as dynamic)['commentCount'] + 1, // Incrementing the commentCount field in the post document
        });
        ...
        if (videoOwnerId != currentUserId) {
          String notificationId = await _createNotification(videoOwnerId, 'Comment',
              'You have a new comment on your video.'); // Creating a notification for the video owner
          ...
          await firestore.collection('users').doc(videoOwnerId).update({
            'notifications': FieldValue.arrayUnion(
                [notificationId]), // Adding the new notification ID to the video owner's notifications list
          });
        }
      }
    } catch (e) {
  ...
  likeComment(String id) async {
    ...
    DocumentSnapshot doc = await firestore
        ...
        .collection('comments')
        .doc(id)
        .get(); // Getting the comment document
    ...
    if ((doc.data()! as dynamic)['likes'].contains(uid)) {
      ...
      await firestore.collection('videos').doc(_postId).collection('comments').doc(id).update({
        'likes': FieldValue.arrayRemove(
            [uid]), // Removing the user's ID from the 'likes' array field in the comment document
      });
    } else {
      await firestore.collection('videos').doc(_postId).collection('comments').doc(id).update({
        'likes':
            FieldValue.arrayUnion([uid]), // Adding the user's ID to the 'likes' array field in the comment document
      });
    }
  }
  ...
  Future<String> _createNotification(String userId, String title, String message) async {
    ...
    var notificationRef = await firestore
        .collection('notifications')
        .add(notification); // Adding the notification document to the 'notifications' collection
